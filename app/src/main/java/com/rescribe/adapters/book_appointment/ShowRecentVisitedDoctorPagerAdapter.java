...
public class ShowRecentVisitedDoctorPagerAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup view, int position) {
        ...
        if (doctorLists.get(position).getDoctorImageUrl().equals("")) {
            String doctorName = doctorLists.get(position).getDocName();
            ...
            Glide.with(mContext)
                    .load(doctorLists.get(position).getDoctorImageUrl())
                    .apply(requestOptions).thumbnail(0.5f)
                    .into(imageURL);
            //--------------
        }

        doctorNameTextView.setText(doctorLists.get(position).getDocName());
